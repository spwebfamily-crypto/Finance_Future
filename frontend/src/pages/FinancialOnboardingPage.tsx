...
const goalOptions: Array<{
...
}> = [
  {
    ...
    copy: "Ter margem para imprevistos.",
    icon: ShieldCheck,
  },
  {
    value: "debt_repayment",
    label: "Reduzir dívidas",
    copy: "Recuperar folga mensal.",
    icon: Landmark,
  },
  {
    value: "home_purchase",
    label: "Comprar casa",
    copy: "Preparar a entrada e custos.",
    icon: Home,
  },
  {
    value: "education",
    label: "Formação",
    copy: "Investir em novas competências.",
    icon: GraduationCap,
  },
  { value: "retirement", label: "Reforma", copy: "Construir no longo prazo.", icon: Clock3 },
  {
    value: "wealth_growth",
    label: "Fazer crescer património",
    copy: "Estudar investimento diversificado.",
    icon: TrendingUp,
  },
  {
    value: "major_purchase",
    label: "Compra importante",
    copy: "Planear sem perder controlo.",
    icon: WalletCards,
  },
  {
    value: "other",
    label: "Outro objetivo",
    copy: "Definir o seu próprio caminho.",
    icon: Sparkles,
  },
];
...
const experienceOptions: Array<{ value: FinancialExperience; label: string; copy: string }> = [
  ...
  {
    value: "advanced",
    label: "Experiência avançada",
    copy: "Conheço riscos, custos e diversificação.",
  },
];
...
const riskOptions: Array<{ value: RiskTolerance; label: string; copy: string }> = [
  {
    value: "conservative",
    label: "Conservador",
    copy: "Prefiro estabilidade, mesmo com menor retorno potencial.",
  },
  { value: "moderate", label: "Moderado", copy: "Aceito alguma oscilação por um horizonte maior." },
  {
    value: "aggressive",
    label: "Agressivo",
    copy: "Aceito quedas fortes e sei que posso perder capital.",
  },
];
...
export function FinancialOnboardingPage() {
  ...
  const monthlyCosts = [numbers.essentials, numbers.housing, numbers.debt].reduce(
  ...
  if (isLoading)
    return (
      <main className="onboarding-page">
        <LoadingState label="A preparar o seu perfil" />
      </main>
    );
  if (loadError) {
    return (
  ...
  return (
    <main className="onboarding-page">
      ...
      <div className="onboarding-shell">
          ...
          >
            {step === 0 && (
              <>
                ...
                <p className="onboarding-step__intro">
                  Use o rendimento líquido habitual. Este valor serve para organizar o contexto —
                  não para decidir automaticamente onde investir.
                </p>
                <label className="money-question">
                  <span>Rendimento líquido mensal</span>
                  <span className="money-question__control">
                    <span aria-hidden="true">€</span>
                    <input
                      ref={(node) => {
                        moneyInputRefs.current.monthlyNetIncome = node;
                      }}
                      inputMode="decimal"
                      value={form.monthlyNetIncome}
                      onChange={(event) => updateMoney("monthlyNetIncome", event.target.value)}
                      placeholder="1 500"
                      aria-invalid={Boolean(invalidFields.monthlyNetIncome)}
                      aria-describedby={`income-help${invalidFields.monthlyNetIncome ? " onboarding-money-error" : ""}`}
                    />
                  </span>
            ...
            {step === 1 && (
              <>
                ...
                <h1 ref={headingRef} tabIndex={-1}>
                  Para onde vai o essencial?
                </h1>
                <p className="onboarding-step__intro">
                  Uma estimativa honesta é suficiente. Não precisa de ser perfeita.
                </p>
                <div className="onboarding-money-grid">
                  <MoneyInput
                    field="monthlyHousingCosts"
                    label="Casa e habitação"
                    value={form.monthlyHousingCosts}
                    onChange={(value) => updateMoney("monthlyHousingCosts", value)}
                    icon={Home}
                    inputRef={(node) => {
                      moneyInputRefs.current.monthlyHousingCosts = node;
                    }}
                    invalid={Boolean(invalidFields.monthlyHousingCosts)}
                  />
                  <MoneyInput
                    field="monthlyEssentialCosts"
                    label="Outras despesas essenciais"
                    value={form.monthlyEssentialCosts}
                    onChange={(value) => updateMoney("monthlyEssentialCosts", value)}
                    icon={WalletCards}
                    inputRef={(node) => {
                      moneyInputRefs.current.monthlyEssentialCosts = node;
                    }}
                    invalid={Boolean(invalidFields.monthlyEssentialCosts)}
                  />
                  <MoneyInput
                    field="monthlyDebtPayments"
                    label="Prestações de dívidas"
                    value={form.monthlyDebtPayments}
                    onChange={(value) => updateMoney("monthlyDebtPayments", value)}
                    icon={Landmark}
                    inputRef={(node) => {
                      moneyInputRefs.current.monthlyDebtPayments = node;
                    }}
                    invalid={Boolean(invalidFields.monthlyDebtPayments)}
                  />
                  <MoneyInput
                    field="currentSavings"
                    label="Poupança disponível hoje"
                    value={form.currentSavings}
                    onChange={(value) => updateMoney("currentSavings", value)}
                    icon={PiggyBank}
                    inputRef={(node) => {
                      moneyInputRefs.current.currentSavings = node;
                    }}
                    invalid={Boolean(invalidFields.currentSavings)}
                  />
                </div>
                <div
                  className={`onboarding-balance ${monthlyMargin < 0 ? "onboarding-balance--warning" : ""}`}
                >
                  <span>Margem mensal declarada</span>
                  <strong>{formatCurrency(monthlyMargin)}</strong>
                  <small>
                    Rendimento menos os custos indicados. Reveja os valores se não refletir a sua
                    realidade.
                  </small>
                </div>
              </>
            ...
            {step === 2 && (
              <>
                ...
                <h1 ref={headingRef} tabIndex={-1}>
                  O que quer priorizar?
                </h1>
                <p className="onboarding-step__intro">
                  O horizonte e o objetivo ajudam a contextualizar a próxima leitura educativa.
                </p>
                ...
                <fieldset className="choice-fieldset">
                  ...
                  <div className="choice-grid choice-grid--three">
                    {horizonOptions.map((option) => (
                      <ChoiceButton
                        key={option.value}
                        selected={form.horizon === option.value}
                        label={option.label}
                        copy={option.copy}
                        onClick={() =>
                          setForm((current) => ({ ...current, horizon: option.value }))
                        }
                      />
                    ))}
                  </div>
            ...
            {step === 3 && (
              <>
                ...
                <p className="onboarding-step__intro">
                  Não há uma resposta certa. Investimentos podem perder valor; este perfil apenas
                  adapta a explicação e nunca executa compras.
                </p>
                <fieldset className="choice-fieldset">
                  ...
                  <div className="choice-grid choice-grid--two">
                    {experienceOptions.map((option) => (
                      <ChoiceButton
                        ...
                        copy={option.copy}
                        onClick={() =>
                          setForm((current) => ({ ...current, experience: option.value }))
                        }
                      />
                    ))}
                  </div>
                ...
                <fieldset className="choice-fieldset">
                  ...
                  <div className="choice-grid choice-grid--three">
                    {riskOptions.map((option) => (
                      <ChoiceButton
                        key={option.value}
                        selected={form.riskTolerance === option.value}
                        label={option.label}
                        copy={option.copy}
                        onClick={() =>
                          setForm((current) => ({ ...current, riskTolerance: option.value }))
                        }
                      />
                    ))}
                  </div>
                ...
                <div className="onboarding-privacy">
                  ...
                  <span>
                    <strong>Os seus valores ficam na sua conta.</strong> Não são partilhados com
                    corretoras nem usados para comprar ativos.
                  </span>
                </div>
              </>
            )}
        ...
        {error && (
          <p className="form-alert onboarding-error" id="onboarding-money-error" role="alert">
            {error}
          </p>
        )}
        ...
        <div className="onboarding-actions">
          ...
          {step < 3 ? (
            <button className="button button--primary" type="button" onClick={continueToNextStep}>
            ...
            >
              {isSaving ? (
                ...
                <>
                  <Check aria-hidden="true" /> Ver orientação educativa
                </>
              )}
            </button>
          )}
        ...
        {existingProfile && (
          <button
            className="onboarding-delete-profile"
            type="button"
...
function MoneyInput({
  ...
  return (
    <label className="onboarding-money-field">
      ...
      <span className="onboarding-money-field__control">
        <span aria-hidden="true">€</span>
        <input
          ref={inputRef}
          name={field}
          ...
          aria-describedby={invalid ? "onboarding-money-error" : undefined}
        />
      </span>
    </label>
  );
