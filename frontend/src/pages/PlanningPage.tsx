...
import { accountApi, analyticsApi, categoryApi, debtApi, incomeApi, recurringExpenseApi, recurringIncomeApi, savingsGoalApi } from '../api/resources';
...
import type { AnalyticsSummary, Category, Debt, FinancialAccount, Income, RecurringExpense, RecurringIncome, SavingsGoal } from '../types';
import { formatCurrency, formatDate, todayInputValue } from '../utils/format';

type DeleteTarget = { type: 'income' | 'goal' | 'recurring' | 'recurringIncome' | 'debt'; id: string; label: string } | null;

const initialIncome = { description: '', source: '', amount: '', date: todayInputValue(), accountId: '' };
const initialGoal = { name: '', targetAmount: '', currentAmount: '', targetDate: '' };
const initialRecurring = { description: '', location: '', amount: '', categoryId: '', accountId: '', dayOfMonth: '' };
const initialRecurringIncome = { description: '', source: '', amount: '', accountId: '', dayOfMonth: '' };
const initialDebt = { name: '', lender: '', currentBalance: '', annualInterestRate: '', monthlyPayment: '', nextPaymentDate: '' };
...
export function PlanningPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const [nextSummary, nextCategories, nextAccounts, nextIncomes, nextGoals, nextRecurring, nextRecurringIncomes, nextDebts] = await Promise.all([
      ...
      setDebtBalances(Object.fromEntries(nextDebts.map((debt) => [debt.id, String(debt.currentBalance)])));
    } catch (requestError) {
  ...
  const totalAccountBalance = useMemo(
    () => accounts.reduce((total, account) => total + (account.currentBalance ?? account.openingBalance), 0),
    [accounts],
  );
  ...
  const upcomingIncomes = useMemo(
    () => recurringIncomes.filter((item) => item.isActive).map((item) => ({ ...item, days: daysUntil(item.nextDueDate) })).sort((a, b) => a.days - b.days),
    [recurringIncomes],
  );
  const calendarItems = useMemo<CalendarItem[]>(() => {
    const horizon = 45;
    const entries: CalendarItem[] = [
      ...upcoming.filter((item) => item.days <= horizon).map((item) => ({ id: `expense-${item.id}`, date: item.nextDueDate, kind: 'expense' as const, title: item.description, detail: `Despesa recorrente · ${dueCopy(item.days)}`, amount: item.amount })),
      ...upcomingIncomes.filter((item) => item.days <= horizon).map((item) => ({ id: `income-${item.id}`, date: item.nextDueDate, kind: 'income' as const, title: item.description, detail: `Rendimento previsto · ${dueCopy(item.days)}`, amount: item.amount })),
      ...debts.filter((debt) => debt.nextPaymentDate && daysUntil(debt.nextPaymentDate) <= horizon).map((debt) => ({ id: `debt-${debt.id}`, date: debt.nextPaymentDate!, kind: 'debt' as const, title: debt.name, detail: `${debt.lender} · ${dueCopy(daysUntil(debt.nextPaymentDate!))}`, amount: debt.monthlyPayment })),
      ...goals.filter((goal) => goal.targetDate && goal.currentAmount < goal.targetAmount && daysUntil(goal.targetDate) <= horizon).map((goal) => ({ id: `goal-${goal.id}`, date: goal.targetDate!, kind: 'goal' as const, title: goal.name, detail: `Meta · ${dueCopy(daysUntil(goal.targetDate!))}` })),
    ];
  ...
  const alerts = useMemo(() => {
    ...
    const urgentDebts = debts.filter((debt) => debt.nextPaymentDate && daysUntil(debt.nextPaymentDate) <= 3);
  ...
  async function createIncome(event: FormEvent) {
    ...
    setIsSaving(true);
    try {
      const created = await incomeApi.create({ description: incomeForm.description, source: incomeForm.source || undefined, amount, date: incomeForm.date, accountId: incomeForm.accountId || null });
  ...
  async function createRecurring(event: FormEvent) {
    ...
    setIsSaving(true);
    try {
      const created = await recurringExpenseApi.create({ ...recurringForm, amount, dayOfMonth, accountId: recurringForm.accountId || null });
  ...
  async function createRecurringIncome(event: FormEvent) {
    ...
    if (!recurringIncomeForm.description.trim() || !Number.isFinite(amount) || amount <= 0 || !Number.isInteger(dayOfMonth) || dayOfMonth < 1 || dayOfMonth > 31) return;
    setIsSaving(true);
    try {
      ...
      setRecurringIncomes((items) => [...items, created].sort((a, b) => a.nextDueDate.localeCompare(b.nextDueDate)));
  ...
  async function recordRecurringIncome(item: RecurringIncome) {
    ...
    try {
      ...
      setRecurringIncomes((items) => items.map((current) => current.id === updated.id ? updated : current));
  ...
  async function toggleRecurringIncome(item: RecurringIncome) {
    ...
    try {
      ...
      setRecurringIncomes((items) => items.map((current) => current.id === updated.id ? updated : current));
      setNotice(item.isActive ? 'Rendimento recorrente colocado em pausa.' : 'Rendimento recorrente reativado.');
    } catch (requestError) { setError(errorMessage(requestError)); }
    finally { setIsSaving(false); }
  }
  ...
  async function createDebt(event: FormEvent) {
    ...
    if (!debtForm.name.trim() || !debtForm.lender.trim() || !Number.isFinite(currentBalance) || currentBalance < 0 || !Number.isFinite(annualInterestRate) || annualInterestRate < 0 || !Number.isFinite(monthlyPayment) || monthlyPayment < 0) return;
    setIsSaving(true);
    try {
      const created = await debtApi.create({ name: debtForm.name, lender: debtForm.lender, currentBalance, annualInterestRate, monthlyPayment, nextPaymentDate: debtForm.nextPaymentDate || null });
  ...
  return (
    <div className="page page--planning">
      ...
      <div className="planning-header-actions">
        <Link className="button button--secondary" to="/accounts"><Landmark aria-hidden="true" /> Contas e cartoes</Link>
        {notificationPermission !== 'unsupported' && notificationPermission !== 'granted' && <button className="button button--secondary" type="button" onClick={() => void enableNotifications()}><Bell aria-hidden="true" /> Ativar alertas</button>}
      </div>
      ...
      <section className="planning-overview" aria-label="Resumo financeiro do mês">
        ...
        <article><span><CreditCard aria-hidden="true" /> Saldo nas contas</span><strong>{formatCurrency(totalAccountBalance, currency)}</strong><small>{accounts.length ? `${accounts.length} conta${accounts.length === 1 ? '' : 's'} ligada${accounts.length === 1 ? '' : 's'}` : 'Adicione uma conta para acompanhar'}</small></article>
      </section>
