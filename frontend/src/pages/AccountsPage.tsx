...
const initialAccount = { name: '', type: 'current' as AccountType, openingBalance: '', creditLimit: '' };
const initialTransfer = { fromAccountId: '', toAccountId: '', amount: '', description: '', date: todayInputValue() };

const accountLabels: Record<AccountType, string> = {
  current: 'À ordem', savings: 'Poupança', cash: 'Dinheiro', credit_card: 'Cartão de crédito', other: 'Outra',
};
...
export function AccountsPage() {
  ...
  const totalBalance = useMemo(() => accounts.reduce((total, account) => total + (account.currentBalance ?? account.openingBalance), 0), [accounts]);
  ...
  async function createAccount(event: FormEvent) {
    ...
    if (!accountForm.name.trim() || !Number.isFinite(openingBalance) || (creditLimit !== undefined && (!Number.isFinite(creditLimit) || creditLimit < 0))) return;
    setIsSaving(true);
    try {
      const created = await accountApi.create({ name: accountForm.name, type: accountForm.type, openingBalance, creditLimit });
  ...
  async function createTransfer(event: FormEvent) {
    ...
    if (!transferForm.fromAccountId || !transferForm.toAccountId || transferForm.fromAccountId === transferForm.toAccountId || !transferForm.date || !Number.isFinite(transferAmount) || transferAmount <= 0) return;
    setIsSaving(true);
    try {
  ...
  if (isLoading) return <div className="page"><LoadingState label="A preparar as suas contas" /></div>;
  if (error && !accounts.length) return <div className="page"><ErrorState message={error} onRetry={() => void load()} /></div>;

  return (
    <div className="page page--accounts">
      <NoticeToast message={notice} onClose={() => setNotice('')} />
      <PageHeader eyebrow="Dinheiro disponível" title="Contas e cartões" description="Acompanhe saldos reais, cartões e transferências sem transformar uma transferência numa despesa." />
      {error && <div className="form-alert form-alert--page" role="alert">{error}</div>}
      <section className="accounts-total"><span>Saldo combinado</span><strong>{formatCurrency(totalBalance, currency)}</strong><small>{accounts.length} {accounts.length === 1 ? 'conta' : 'contas'} registadas</small></section>

      <div className="accounts-grid">
        <section className="accounts-panel" aria-labelledby="account-create-title"><div className="section-heading"><div><p className="eyebrow">Adicionar</p><h2 id="account-create-title">Nova conta</h2></div></div><form className="planning-form" onSubmit={createAccount}><label className="field"><span>Nome</span><input value={accountForm.name} onChange={(event) => setAccountForm((form) => ({ ...form, name: event.target.value }))} placeholder="Ex.: Conta principal" /></label><label className="field"><span>Tipo</span><select value={accountForm.type} onChange={(event) => setAccountForm((form) => ({ ...form, type: event.target.value as AccountType, creditLimit: event.target.value === 'credit_card' ? form.creditLimit : '' }))}>{Object.entries(accountLabels).map(([type, label]) => <option key={type} value={type}>{label}</option>)}</select></label><label className="field"><span>Saldo inicial</span><input inputMode="decimal" value={accountForm.openingBalance} onChange={(event) => setAccountForm((form) => ({ ...form, openingBalance: event.target.value }))} placeholder="0,00" /></label>{accountForm.type === 'credit_card' && <label className="field"><span>Limite do cartão</span><input inputMode="decimal" value={accountForm.creditLimit} onChange={(event) => setAccountForm((form) => ({ ...form, creditLimit: event.target.value }))} placeholder="1 500" /></label>}<button className="button button--accent" type="submit" disabled={isSaving}>{isSaving ? <Spinner label="A guardar" /> : <><Plus aria-hidden="true" /> Criar conta</>}</button></form></section>
        <section className="accounts-panel" aria-labelledby="transfer-title"><div className="section-heading"><div><p className="eyebrow">Mover dinheiro</p><h2 id="transfer-title">Transferência</h2></div><ArrowRightLeft aria-hidden="true" /></div><form className="planning-form" onSubmit={createTransfer}><div className="planning-form__split"><label className="field"><span>De</span><select value={transferForm.fromAccountId} onChange={(event) => setTransferForm((form) => ({ ...form, fromAccountId: event.target.value }))}><option value="">Escolher</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label className="field"><span>Para</span><select value={transferForm.toAccountId} onChange={(event) => setTransferForm((form) => ({ ...form, toAccountId: event.target.value }))}><option value="">Escolher</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label></div><div className="planning-form__split"><label className="field"><span>Valor</span><input inputMode="decimal" value={transferForm.amount} onChange={(event) => setTransferForm((form) => ({ ...form, amount: event.target.value }))} placeholder="0,00" /></label><label className="field"><span>Data</span><input type="date" value={transferForm.date} onChange={(event) => setTransferForm((form) => ({ ...form, date: event.target.value }))} /></label></div><label className="field"><span>Nota <em>opcional</em></span><input value={transferForm.description} onChange={(event) => setTransferForm((form) => ({ ...form, description: event.target.value }))} placeholder="Ex.: reforço da poupança" /></label><button className="button button--primary" disabled={isSaving || accounts.length < 2} type="submit">Transferir</button></form></section>
      </div>

      <section className="accounts-panel accounts-panel--list" aria-labelledby="accounts-title"><div className="section-heading"><div><p className="eyebrow">Saldos</p><h2 id="accounts-title">As suas contas</h2></div></div><div className="account-cards">{accounts.map((account) => { const balance = account.currentBalance ?? account.openingBalance; const cardUse = account.type === 'credit_card' && account.creditLimit ? Math.max(0, -balance) / account.creditLimit : null; return <article className={`account-card-large account-card-large--${account.type}`} key={account.id}><span className="account-card-large__icon"><AccountIcon type={account.type} /></span><div><p>{accountLabels[account.type]}</p><h3>{account.name}</h3></div><strong>{formatCurrency(balance, currency)}</strong>{cardUse !== null && <small>{Math.min(100, cardUse * 100).toFixed(0)}% do limite utilizado</small>}<button type="button" className="icon-button icon-button--danger" aria-label={`Remover conta ${account.name}`} onClick={() => setDeleteTarget(account)}><Trash2 aria-hidden="true" /></button></article>; })}</div></section>
