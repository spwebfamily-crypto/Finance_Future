...
vi.mock('../auth/AuthContext', () => ({
  useAuth: () => ({ user: { id: 'user-1', name: 'Rita', email: 'rita@example.com', currency: 'EUR' } }),
}));
...
describe('AccountsPage account actions', () => {
  ...
  it('lets the user correct the displayed account balance', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: 'Corrigir saldo da conta Conta principal' }));
    ...
    await waitFor(() => expect(api.correctBalance).toHaveBeenCalledWith('account-1', { currentBalance: 250.5 }));
  ...
  it('deletes an account even when the server refreshes related transfer data', async () => {
    ...
    expect(screen.getByText(/As transferências desta conta também serão removidas/)).toBeInTheDocument();
