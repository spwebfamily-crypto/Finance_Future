    ...
    <div className="page page--dashboard">
      ...
      {loading ? <LoadingState label="A preparar a sua visão geral" /> : error && !summary ? <ErrorState message={error} onRetry={() => void load()} /> : !summary ? <EmptyState title="Ainda sem resumo" description="Adicione despesas para começar a ver a análise mensal." /> : <>
        ...
        <div className="dashboard-grid">
          <section className="dashboard-panel dashboard-panel--categories" aria-labelledby="categories-chart-title"><div className="section-heading"><div><p className="eyebrow">Distribuição</p><h2 id="categories-chart-title">Por categoria</h2></div></div>{categoryData.length ? <><div className="dashboard-chart" role="img" aria-label="Gráfico de barras das despesas por categoria"><ResponsiveContainer width="100%" height="100%"><BarChart data={categoryData} layout="vertical" margin={{ top: 4, right: 12, left: 0, bottom: 4 }}><XAxis type="number" hide /><YAxis type="category" dataKey="name" width={82} tickLine={false} axisLine={false} tick={{ fill: '#6b776f', fontSize: 12 }} /><Tooltip formatter={(value) => formatCurrency(Number(Array.isArray(value) ? value[0] : value ?? 0), currency)} cursor={{ fill: '#f4f6f1' }} isAnimationActive={!reduceMotion} /><Bar dataKey="amount" radius={[0, 6, 6, 0]} isAnimationActive={!reduceMotion}>{categoryData.map((entry) => <Cell key={entry.category.id} fill={entry.fill} />)}</Bar></BarChart></ResponsiveContainer></div><CategoryTable data={categoryData} currency={currency} /></> : <EmptyState title="Sem despesas neste mês" description="As categorias aparecerão aqui quando existirem movimentos." />}</section>
          <section className="dashboard-panel dashboard-panel--trend" aria-labelledby="trend-chart-title"><div className="section-heading"><div><p className="eyebrow">Últimos 6 meses</p><h2 id="trend-chart-title">Evolução mensal</h2></div></div>{trendData.length ? <><div className="dashboard-chart" role="img" aria-label="Gráfico de linha do total mensal"><ResponsiveContainer width="100%" height="100%"><LineChart data={trendData} margin={{ top: 10, right: 10, left: -22, bottom: 0 }}><CartesianGrid vertical={false} stroke="#dce3dc" /><XAxis dataKey="label" tickLine={false} axisLine={false} tick={{ fill: '#6b776f', fontSize: 12 }} /><YAxis tickFormatter={(value: number) => `${value}€`} tickLine={false} axisLine={false} tick={{ fill: '#8c9790', fontSize: 11 }} /><Tooltip formatter={(value) => formatCurrency(Number(Array.isArray(value) ? value[0] : value ?? 0), currency)} isAnimationActive={!reduceMotion} /><Line type="monotone" dataKey="total" stroke="#236e57" strokeWidth={3} dot={{ fill: '#c9f277', stroke: '#164a3c', strokeWidth: 2, r: 3 }} activeDot={{ r: 5 }} isAnimationActive={!reduceMotion} /></LineChart></ResponsiveContainer></div><TrendTable data={trendData} currency={currency} /></> : <EmptyState title="Tendência indisponível" description="São necessários movimentos para construir a série mensal." />}</section>
        </div>
