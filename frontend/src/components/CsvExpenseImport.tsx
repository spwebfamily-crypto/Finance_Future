...
export function CsvExpenseImport({ categories, accounts, onImported }: CsvExpenseImportProps) {
    ...
    reader.onload = () => {
      ...
      const parsed = table.slice(1).flatMap((source) => {
        ...
        const mappedCategory =
          categoryByName.get(normalizeHeader(valueAt(source, map.category))) || "";
        return [
          {
            date,
            description,
            ...
            location: valueAt(source, map.location) || location,
          },
        ];
      });
      if (!parsed.length) {
  ...
  if (!isOpen)
    return (
      ...
      >
        <Upload aria-hidden="true" /> Importar CSV
      </button>
    );
    ...
    <section className="csv-import" aria-labelledby="csv-import-title">
      ...
      <div className="csv-import__controls">
        ...
        <label className="field field--compact">
          <span>Categoria de reserva</span>
          <select
      ...
      {rows.length > 0 && (
        <>
          <div className="csv-import__preview">
            ...
            {rows.length > 5 && <small>e mais {rows.length - 5} movimentos.</small>}
          </div>
          <button
            className="button button--primary"
            type="button"
            disabled={isImporting || !categories.length}
