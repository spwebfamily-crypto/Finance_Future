...
const markVariants: Variants = {
  ...
  idle: { opacity: 1, scale: 1, y: 0, rotate: 0, transition: { duration: 0.16, ease: [0.22, 1, 0.36, 1] } },
  loading: {
    opacity: [0.75, 1, 1],
    scale: [0.94, 1.025, 1],
    y: [3, 0, 0],
...
export function Brand({ compact = false, linked = true, phase = 'idle' }: BrandProps) {
  ...
  const content = (
    ...
    >
      <motion.span className="brand__mark" aria-hidden="true" variants={reduceMotion ? undefined : markVariants}>
