...
export function PdfReceiptPreview({ file, href, title = 'Comprovativo PDF' }: PdfReceiptPreviewProps) {
  ...
  useEffect(() => {
    ...
    const destroyOwnedResource = () => {
      ...
      destroyPromise = (ownedDocument ? ownedDocument.destroy() : loadingTask?.destroy() ?? Promise.resolve())
        .catch(() => undefined);
    ...
    void (async () => {
      ...
      if (!active || generation !== loadGenerationRef.current) return;
      loadingTask = pdfjs.getDocument({ data: bytes, isEvalSupported: false, useSystemFonts: true });
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!active || generation !== renderGenerationRef.current || document !== documentRef.current) return;
      ...
      if (estimatedPixels > MAX_PREVIEW_PIXELS) renderScale *= Math.sqrt(MAX_PREVIEW_PIXELS / estimatedPixels);
      ...
      if (active && generation === renderGenerationRef.current && document === documentRef.current) setStatus('ready');
    })().catch((error: unknown) => {
      if (active
        && generation === renderGenerationRef.current
        && !(error instanceof Error && error.name === 'RenderingCancelledException')) setStatus('error');
    }).finally(() => {
      page?.cleanup();
  ...
  return (
    <div className={`receipt-viewer ${status === 'error' ? 'receipt-viewer--error' : ''}`}>
      <div className="receipt-viewer__stage">
        {status === 'loading' && <div className="receipt-viewer__skeleton" aria-label="A preparar pré-visualização do PDF"><FileText aria-hidden="true" /></div>}
        <canvas ref={canvasRef} className="receipt-viewer__canvas" role="img" aria-label={`${title}, página ${pageNumber} de ${pageCount || 1}`} />
        {status === 'ready' && <span className="receipt-viewer__badge">PDF · {pageNumber}/{pageCount}</span>}
        {status === 'error' && <div className="receipt-viewer__fallback"><FileText aria-hidden="true" /><strong>Não foi possível mostrar esta página.</strong><span>O ficheiro continua disponível.</span></div>}
      </div>
      <div className="receipt-viewer__toolbar">
        <div className="receipt-viewer__pager" aria-label="Navegação das páginas do PDF">
          <button type="button" className="icon-button" onClick={() => setPageNumber((current) => Math.max(1, current - 1))} disabled={pageNumber <= 1} aria-label="Página anterior"><ChevronLeft aria-hidden="true" /></button>
          <span>Página {pageNumber} de {pageCount || 1}</span>
          <button type="button" className="icon-button" onClick={() => setPageNumber((current) => Math.min(pageCount, current + 1))} disabled={!pageCount || pageNumber >= pageCount} aria-label="Página seguinte"><ChevronRight aria-hidden="true" /></button>
        </div>
        <a className="text-button" href={href} target="_blank" rel="noreferrer">Abrir PDF <ExternalLink aria-hidden="true" /></a>
      </div>
    </div>
  );
}
