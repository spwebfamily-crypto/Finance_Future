...
const strongTotal = /\b(total a pagar|montante a pagar|valor a pagar|importe a pagar|total do documento|total da fatura|total fatura|total liquido|total geral|grand total|amount due|payment due|balance due|saldo a pagar)\b/;
const ordinaryTotal = /\b(total|valor total|montante total|importe total|total pago|valor pago)\b/;
const excludedAmount = /\b(subtotal|sub total|total sem iva|total iva|iva|vat|imposto|taxa|base tributavel|incidencia|desconto|poupanca|troco|entregue|recebido|retencao|arredondamento)\b/;
const unitAmount = /\b(preco unitario|valor unitario|unitario|quantidade|qtd|unit price|preco un)\b/;

function amountLabelScore(line: ReceiptLine, matchIndex: number, matchLength: number, previous?: ReceiptLine) {
  const prefix = normalizeText(line.raw.slice(Math.max(0, matchIndex - 72), matchIndex));
  const suffix = normalizeText(line.raw.slice(matchIndex + matchLength, matchIndex + matchLength + 36));
  ...
  if (/(?:total a pagar|montante a pagar|valor a pagar|importe a pagar|total do documento|total da fatura|total liquido|total geral|grand total|amount due|payment due|balance due|saldo a pagar)\s*$/.test(prefix)) return 190;
  if (/(?:subtotal|sub total|total sem iva|total iva|iva|vat|imposto|taxa|base tributavel|incidencia|desconto|poupanca|troco|entregue|recebido|retencao|arredondamento)\s*$/.test(prefix)) return -180;
  if (/(?:preco unitario|valor unitario|unitario|quantidade|qtd|unit price|preco un)\s*$/.test(prefix)) return -110;
  if (/(?:valor total|montante total|importe total|total pago|valor pago|total)\s*$/.test(prefix)) return 125;
  ...
  const lineMoneyCount = [...line.raw.matchAll(new RegExp(MONEY_PATTERN.source, MONEY_PATTERN.flags))].length;
  ...
  if (lineMoneyCount === 1 && ordinaryTotal.test(line.normalized) && !/\bsubtotal\b/.test(line.normalized)) return 105;

  if (/^(?:total a pagar|montante a pagar|valor a pagar|importe a pagar|total do documento|total da fatura|total liquido|grand total|amount due|payment due|balance due|saldo a pagar)(?: eur)?$/.test(previousText)) return 170;
  if (/^(?:subtotal|sub total|total iva|iva|vat|imposto|desconto|troco)$/.test(previousText)) return -140;
  if (/^(?:valor total|montante total|importe total|total pago|valor pago|total)(?: eur)?$/.test(previousText)) return 110;
...
function parseAmount(text: string): ParsedValue<string> {
  ...
  lines.forEach((line, lineIndex) => {
    for (const match of line.raw.matchAll(new RegExp(MONEY_PATTERN.source, MONEY_PATTERN.flags))) {
      ...
      const labelScore = amountLabelScore(line, match.index ?? 0, match[0].length, lines[lineIndex - 1]);
      ...
      const nearbyRaw = line.raw.slice(Math.max(0, (match.index ?? 0) - 5), (match.index ?? 0) + match[0].length + 5);
  ...
  const sorted = [...eligible].sort((left, right) => right.score - left.score || right.value - left.value || right.index - left.index);
  const best = sorted[0];
  const competing = sorted.find((candidate) => candidate.value !== best.value);
  let confidence = best.score >= 175 ? 0.99 : best.score >= 110 ? 0.94 : best.score >= 60 ? 0.82 : 0.58;
...
const issueDateLabel = /\b(data de emissao|data emissao|emitido em|data da fatura|data fatura|data do documento|document date|issue date)\b/;
const excludedDateLabel = /\b(data de vencimento|vencimento|validade|limite de pagamento|pagar ate|due date|data entrega|entrega prevista)\b/;
...
function dateLabelScore(line: ReceiptLine, matchIndex: number, previous?: ReceiptLine) {
  ...
  const previousText = previous?.normalized ?? '';
  if (/(?:data de vencimento|vencimento|validade|limite de pagamento|pagar ate|due date|data entrega|entrega prevista)\s*$/.test(prefix)) return -175;
  if (/(?:data de emissao|data emissao|emitido em|data da fatura|data fatura|data do documento|document date|issue date)\s*$/.test(prefix)) return 135;
...
function parseDate(text: string): ParsedValue<string> {
  ...
  lines.forEach((line, lineIndex) => {
    ...
    const push = (year: number, month: number, day: number, matchIndex: number) => {
      ...
      candidates.push({ value: isoDate(year, month, day), score: baseScore + dateLabelScore(line, matchIndex, previous), index: lineIndex });
    };

    const isoPattern = new RegExp(`\\b(${digit}{4})\\s*[-/.]\\s*(${digit}{1,2})\\s*[-/.]\\s*(${digit}{1,2})\\b`, 'g');
    for (const match of line.raw.matchAll(isoPattern)) {
      push(Number(normalizeOcrDigits(match[1])), Number(normalizeOcrDigits(match[2])), Number(normalizeOcrDigits(match[3])), match.index ?? 0);
    }

    const localPattern = new RegExp(`\\b(${digit}{1,2})\\s*[./-]\\s*(${digit}{1,2})\\s*[./-]\\s*(${digit}{2}|${digit}{4})\\b`, 'g');
    for (const match of line.raw.matchAll(localPattern)) {
      push(fullYear(match[3]), Number(normalizeOcrDigits(match[2])), Number(normalizeOcrDigits(match[1])), match.index ?? 0);
    }

    const normalized = line.normalized;
    const wordDigit = '[0-9oqdil|sb]';
    const wordsPattern = new RegExp(`\\b(${wordDigit}{1,2})\\s+(?:de\\s+)?([a-z]+)\\s+(?:de\\s+)?(${wordDigit}{2}|${wordDigit}{4})\\b`, 'g');
    for (const match of normalized.matchAll(wordsPattern)) {
      const month = monthNumbers[match[2]];
      if (month) push(fullYear(match[3]), month, Number(normalizeOcrDigits(match[1])), match.index ?? 0);
    }
  });

  const eligible = candidates.filter((candidate) => candidate.score > -30);
  if (!eligible.length) return { confidence: 0 };
  const sorted = [...eligible].sort((left, right) => right.score - left.score || left.index - right.index);
  const best = sorted[0];
  const competing = sorted.find((candidate) => candidate.value !== best.value);
  let confidence = best.score >= 120 ? 0.98 : best.score >= 65 ? 0.9 : best.score >= 25 ? 0.78 : 0.6;
  if (competing && best.score - competing.score < 18) confidence = Math.min(confidence, 0.7);
...
const addressWords = /\b(rua|avenida|av|largo|travessa|estrada|praca|alameda|rotunda|calcada|centro comercial)\b/;
const documentMetadata = /\b(pagina|page|fatura|invoice|fatura recibo|recibo|talao|nif|nipc|contribuinte|documento|original|duplicado|data|hora|caixa|operador|cliente|consumidor final|iban|swift|atcud)\b/;
...
function parseMerchant(lines: ReceiptLine[]): ParsedValue<string> {
  ...
  for (const line of lines) occurrences.set(line.normalized, (occurrences.get(line.normalized) ?? 0) + 1);
  ...
  for (const line of lines) {
    ...
    if (/\b(lda|sa|unipessoal|limitada|store|shop|mercado|supermercado|farmacia|restaurante|cafe|hotel)\b/.test(line.normalized)) score += 16;
    ...
    if (/^[\d\W]+$/.test(line.raw) || /\b(qtd|quantidade|preco|artigo|codigo|ref)\b/.test(line.normalized)) score -= 45;
...
function parseLocation(lines: ReceiptLine[]): ParsedValue<string> {
  ...
  lines.forEach((line, lineIndex) => {
    ...
    const hasLocationLabel = /^(morada|endereco|local|loja|estabelecimento)\b/.test(line.normalized);
    ...
    if (/\b(cliente|morada de faturacao|faturar a|destinatario|bill to|ship to)\b/.test(`${previous?.normalized ?? ''} ${line.normalized}`)) score -= 150;
  ...
  return { value: best.value, confidence: best.score >= 105 ? 0.94 : best.score >= 75 ? 0.84 : 0.68 };
}
...
const categoryFamilies: CategoryFamily[] = [
  {
    ...
    signals: [
      [/\b(supermercado|hipermercado|mercearia|mercado|restaurante|cafeteria|cafe|padaria|pastelaria|talho|refeicao|continente|pingo doce|lidl|aldi|auchan|intermarche)\b/, 95],
  ...
  },
  {
    ...
    signals: [
      [/\b(combustivel|gasolina|gasoleo|portagem|estacionamento|metro|comboio|autocarro|fertagus)\b/, 100],
  ...
  },
  {
    ...
    signals: [
      [/\b(renda|aluguer|condominio|eletricidade|energia|agua|internet|telecomunicacoes|mobiliario|ikea|leroy merlin)\b/, 95],
  ...
  },
  {
    ...
    signals: [
      [/\b(farmacia|hospital|clinica|consulta|dentista|medico|medica|medicamento|analises|laboratorio|saude)\b/, 100],
    ],
  },
  {
    categoryName: /\b(lazer|entretenimento)\b/,
    signals: [
      [/\b(cinema|teatro|concerto|festival|streaming|netflix|spotify|lazer|museu|parque tematico)\b/, 95],
      [/\b(bilhete|jogo)\b/, 52],
    ],
  },
  {
...
function inferCategory(text: string, categories: Category[]): ParsedValue<string> {
  ...
  const scored = categories.map((category) => {
    ...
    let score = 0;
    if (namePattern && new RegExp(`\\b(?:categoria|category)\\s*:?\\s*${namePattern.source}`).test(normalized)) score += 145;
    ...
    if (family) {
      for (const [pattern, weight] of family.signals) if (pattern.test(normalized)) score += weight;
    }
  ...
  const confidence = best.score >= 140 ? 0.98 : best.score >= 100 && margin >= 25 ? 0.92 : best.score >= 80 ? 0.84 : 0.72;
  return { value: best.category.id, confidence };
}

function imageElement(file: File): Promise<{ source: CanvasImageSource; width: number; height: number; cleanup: () => void }> {
...
async function prepareImageForOcr(file: File): Promise<{ source: File | HTMLCanvasElement; cleanup: () => void }> {
  ...
  try {
    ...
    try {
      ...
      const smallImageBoost = Math.min(1.5, 1200 / Math.max(Math.min(decoded.width, decoded.height), 1));
