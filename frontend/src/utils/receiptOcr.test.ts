...
describe('receipt OCR parsing', () => {
  ...
  it('distinguishes a medical station and the phrase square metre from transport', () => {
    ...
    expect(parseReceiptText('POSTO MÉDICO CENTRAL\nConsulta 30,00', categories).categoryId).toBe('health');
    expect(parseReceiptText('METRO QUADRADO DECORAÇÃO\nTOTAL 30,00', categories).categoryId).toBeUndefined();
  });
  ...
  it('matches a short custom category only when explicitly labelled', () => {
    ...
    expect(parseReceiptText('PETSHOP PETS & CIA\nTOTAL 9,90', categories).categoryId).toBeUndefined();
  });
