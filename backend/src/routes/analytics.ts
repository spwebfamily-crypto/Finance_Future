...
import { calculateSpendingLevel, currentMonthContext, moneyNumber, monthBounds, monthsEndingAt, shiftMonth } from '../services/analyticsService.js';
...
async function userContext(userId: string) {
  const user = await prisma.user.findUnique({ where: { id: userId }, select: { timeZone: true, currency: true } });
  const timeZone = user?.timeZone ?? 'Europe/Lisbon';
  return { ...currentMonthContext(new Date(), timeZone), timeZone, currency: user?.currency ?? 'EUR' };
}

function validateSelectedMonth(month: string, currentMonth: string, response: Parameters<typeof sendError>[0]) {
...
router.get('/summary', async (request: AuthenticatedRequest, response, next) => {
  try {
    ...
    const [{ start, end }, previousBounds, categoryData] = await Promise.all([
      Promise.resolve(monthBounds(month)), Promise.resolve(monthBounds(previousMonth)), categoriesAndBudgets(request.user!.id),
    ]);
    const { categories } = categoryData;
    const [expenses, previousExpenses] = await Promise.all([
      prisma.expense.findMany({ where: { userId: request.user!.id, date: { gte: start, lt: end } }, select: { categoryId: true, amount: true } }),
      prisma.expense.findMany({ where: { userId: request.user!.id, date: { gte: previousBounds.start, lt: previousBounds.end } }, select: { categoryId: true, amount: true } }),
    ]);
    const amounts = new Map<string, Prisma.Decimal>();
    const previousAmounts = new Map<string, Prisma.Decimal>();
    for (const expense of expenses) amounts.set(expense.categoryId, (amounts.get(expense.categoryId) ?? new Prisma.Decimal(0)).add(expense.amount));
    for (const expense of previousExpenses) previousAmounts.set(expense.categoryId, (previousAmounts.get(expense.categoryId) ?? new Prisma.Decimal(0)).add(expense.amount));
    ...
    return response.json({ data: {
      month, timeZone: context.timeZone, currency: context.currency, total: moneyNumber(total),
      previousMonthTotal: moneyNumber(previousTotal), changeAmount: moneyNumber(difference),
      changePercent: previousTotal.isZero() ? null : moneyNumber(difference.div(previousTotal).mul(100)),
      byCategory: categories.map((category) => {
        ...
        return {
          ...
          changePercent: previousAmount.isZero() ? null : moneyNumber(changeAmount.div(previousAmount).mul(100)),
...
router.get('/levels', async (request: AuthenticatedRequest, response, next) => {
  try {
    ...
    const [{ categories, budgets }, expenses] = await Promise.all([
      ...
      prisma.expense.findMany({ where: { userId: request.user!.id, date: { gte: historyStart, lt: end } }, select: { categoryId: true, amount: true, date: true } }),
    ]);
    ...
    for (const expense of expenses) {
      ...
      categoryValues.set(expenseMonth, (categoryValues.get(expenseMonth) ?? new Prisma.Decimal(0)).add(expense.amount));
    ...
    const daysInMonth = isCurrentMonth ? context.daysInMonth : new Date(selectedBounds.end.getTime() - 1).getUTCDate();
    return response.json({ data: categories.map((category) => {
      ...
      const historyAmounts = historyMonths.map((historyMonth) => categoryValues.get(historyMonth) ?? new Prisma.Decimal(0));
      const budget = budgets.get(category.id);
      const calculation = calculateSpendingLevel({ currentAmount, historyAmounts, monthlyLimit: budget?.monthlyLimit, isCurrentMonth, elapsedDays: isCurrentMonth ? context.elapsedDays : daysInMonth, daysInMonth });
      return {
        category: { id: category.id, name: category.name, icon: category.icon },
        ...
        baselineAmount: calculation.baselineAmount === null ? null : moneyNumber(calculation.baselineAmount),
        comparisonRatio: calculation.comparisonRatio === null ? null : calculation.comparisonRatio.toDecimalPlaces(4).toNumber(),
        budget: budget ? {
          id: budget.id,
        ...
        history: historyMonths.map((historyMonth, index) => ({ month: historyMonth, amount: moneyNumber(historyAmounts[index]!) })),
      };
    }) });
  } catch (error) { return next(error); }
});
...
router.get('/trend', async (request: AuthenticatedRequest, response, next) => {
  try {
    ...
    const [expenses, categories] = await Promise.all([
      prisma.expense.findMany({ where: { userId: request.user!.id, date: { gte: start, lt: end } }, select: { categoryId: true, amount: true, date: true } }),
      prisma.category.findMany({ where: { userId: request.user!.id }, orderBy: { name: 'asc' }, select: { id: true, name: true, icon: true } }),
    ]);
    const values = new Map<string, Map<string, Prisma.Decimal>>();
    for (const expense of expenses) {
      ...
      monthValues.set(expense.categoryId, (monthValues.get(expense.categoryId) ?? new Prisma.Decimal(0)).add(expense.amount));
      values.set(month, monthValues);
    }
    return response.json({ data: { timeZone: context.timeZone, currency: context.currency, series: monthKeys.map((month) => {
      const monthValues = values.get(month) ?? new Map<string, Prisma.Decimal>();
      return { month, total: moneyNumber(sum([...monthValues.values()])), categories: categories.map((category) => ({ category: { id: category.id, name: category.name, icon: category.icon }, amount: moneyNumber(monthValues.get(category.id) ?? new Prisma.Decimal(0)) })) };
    }) } });
  } catch (error) { return next(error); }
});
