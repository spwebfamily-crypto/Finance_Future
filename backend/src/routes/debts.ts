...
router.get('/', async (request: AuthenticatedRequest, response, next) => {
  try {
    const debts = await prisma.debt.findMany({ where: { userId: request.user!.id }, select: debtSelect, orderBy: [{ nextPaymentDate: 'asc' }, { createdAt: 'desc' }] });
...
router.post('/', async (request: AuthenticatedRequest, response, next) => {
  try {
    ...
    const debt = await prisma.debt.create({ data: { ...input, userId: request.user!.id }, select: debtSelect });
...
router.patch('/:id', async (request: AuthenticatedRequest, response, next) => {
  try {
    ...
    const existing = await prisma.debt.findFirst({ where: { id: request.params.id, userId: request.user!.id }, select: { id: true } });
    if (!existing) return sendError(response, 404, 'DEBT_NOT_FOUND', 'Dívida não encontrada.');
    const debt = await prisma.debt.update({ where: { id: existing.id }, data: input, select: debtSelect });
