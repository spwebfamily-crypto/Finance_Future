...
type PublicRecurringIncome = Prisma.RecurringIncomeGetPayload<{
  select: typeof recurringIncomeSelect;
}>;
  ...
  try {
    ...
    const recurringIncome = await prisma.recurringIncome.create({
      data: {
        ...
        nextDueDate: nextDueDate(input.dayOfMonth),
      },
      select: recurringIncomeSelect,
    });
  ...
  try {
    ...
    if (!existing)
      return sendError(
        ...
        "Rendimento recorrente não encontrado.",
      );
    if (input.accountId && !(await accountBelongsToUser(input.accountId, request.user!.id))) {
      return sendError(response, 404, "ACCOUNT_NOT_FOUND", "Conta não encontrada.");
    }
  ...
  try {
    const recurringIncome = await prisma.$transaction(async (transaction) => {
      ...
      return transaction.recurringIncome.update({
        ...
        data: {
          ...
          nextDueDate: advanceDueDate(current.nextDueDate, current.dayOfMonth),
        },
        select: recurringIncomeSelect,
      });
    });
    if (!recurringIncome)
      return sendError(
        ...
        404,
        "RECURRING_INCOME_NOT_FOUND",
        "Rendimento recorrente ativo não encontrado.",
      );
    return response.json({ data: presentRecurringIncome(recurringIncome) });
  } catch (error) {
    return next(error);
